...
if __name__ == '__main__':
    ...
    for time_id in time_ids:
        ...
        node_set_lists[time_id], feature_set_lists[time_id], ano_label_lists[time_id] = select_Node_emebdding(
            graph_list[time_id], diff_features[time_id], snapshot_lists[time_id], ano_node_lists[time_id])
        ...
        if time_id != time_ids[-1]:
            ...
            if original_ano_features_lists[time_id].size==0:
                ...
                if(count_ano(ano_label_lists[time_id])>0):
                    autoencoder.train(original_ano_features_lists[time_id], num_epochs=200, learning_rate=0.001)
                    generated_features_lists[time_id] = autoencoder.generate_abnormal_features(
                        original_ano_features_lists[time_id], num_samples_lists[time_id])
                    ...
                    ano_all_label_lists[time_id] = np.concatenate(
                        (ano_label_lists[time_id], generated_ano_list[time_id]))
                    # 完整异常标签
...
with open("data_AE_change_networkshot/"+str(n)+dataname + 'train.txt', 'w') as f:
...
with open("data_AE_change_networkshot/"+str(n)+dataname + 'valid.txt', 'w') as f:
    for i in np.arange(len(valid_label)):
        label = str(int(valid_label[i]))
        embedding = ','.join([str(x) for x in valid_infor[i]])
        f.write(f"{label},{embedding}\n")
with open("data_AE_change_networkshot/"+str(n)+dataname + 'test.txt', 'w') as f:
