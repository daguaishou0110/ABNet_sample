...
#        print("edge_embbing.shape:",edge_embbing.shape)
        ...
        feature=0.9*generality_feature+0.1*target_feature
...
def train_Adversarial_Model(dataset, train_loader, valid_loader, model, criterion,initial_learning_rate):
    model_path = 'output_AE/' + dataset + '_model/'
    ...
    for epoch in range(epochs + 1):
        ...
        # scheduler=StepLR(inner_optimizer, step_size=1,gamma=0.5)
        model.train()
        loss_vec = []
        auc_vec = []
...
#            print("link_label",link_label.shape)
        ...
        torch.save(model.state_dict(),model_path + 'model' + str(epoch) + '.pkl')
...
#        if epoch % 10 == 0:
        print(
                'Adversarial Model Epoch: [{}/{}], learning rate:{:.6f}, train loss:{:.4f}, train auc:{:.4f}, valid auc:{:.4f}'.format(
                    epoch, epochs, learning_rate, loss, auc, valid_auc))
...
def test_Adversarial_Model(test_loader, adversarial_model, best_valid_dir):
    ...
    y_true = []
    aupr_vec=[]
    incorrect_node_features = []  # 用于保存错误预测节点的特征
    for i, data in enumerate(test_loader):
        ...
        # 记录错误预测的节点索引
        incorrect_indices = np.where(pred != link_label)[0]

        # 提取错误预测节点的特征
        incorrect_features = infor[incorrect_indices].cpu().numpy()  # 假设 infor 是节点特征
        incorrect_node_features.append(incorrect_features)  # 保存错误预测的节点特征
    ...
    # total_samples = len(link_label)
    # error_rate = num_errors / total_samples
    # fp_rate = num_fp / total_samples
    # fn_rate = num_fn / total_samples
    ...
    np.save('pred.npy', pred.cpu().numpy())  # 将 Tensor 转为 numpy
    np.save('link_label.npy', link_label.cpu().numpy())
    np.save('incorrect_node_features.npy', np.vstack(incorrect_node_features))  # 合并保存
...
outputpath = './output_AE/'
dataset = 'reddit'
if not os.path.exists(outputpath):
    os.mkdir(outputpath)
outfile = open('./output_AE/out_'+dataset+'_'+''+'.txt', 'w', encoding='utf-8')
...
dataname=dataset
train_data = f'data_AE/'+dataname+'train.txt'
valid_data = f'data_AE/'+dataname+'valid.txt'
test_data=f'data_AE/'+dataname+'test.txt'
...
for repeat in range(repeats):
    ...
    model_directory = 'output_AE/'+dataset+'_model'
...
#        print(write_infor)
        write_infor = 'model_file:{},repeat:{}, ROC-AUC:{:.4f}, Precision:{:.4f}, Accuracy:{:.4f}, F1_score:{:.4f}, AUPR:{:.4f}\n'.format(
            model_file,repeat + 1, auc, precision, acc, f1, aupr)
        print(write_infor)
        # print("error rate",error_rate,"fp_rate",fp,"fn_rate",fn_rate)
